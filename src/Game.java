...
// Lớp chính điều khiển toàn bộ game, kế thừa từ Application của JavaFX
public class Game extends Application {

    //PHẦN THÊM VÀO
    private BorderPane mainPane;  // Layout chính
    private boolean isBasicMode;  // Biến xác định chế độ (true cho Basic, false cho Graphics)
    private Deck deck;  // Bộ bài (giả sử bạn đã có lớp Deck)

    // Constructor nhận tham số isBasicMode để xác định chế độ hiển thị
    public Game(boolean isBasicMode) {
        this.isBasicMode = isBasicMode;
        this.mainPane = new BorderPane();  // Khởi tạo BorderPane cho mainPane
        this.deck = new Deck();  // Giả sử Deck là lớp quản lý bộ bài
    } //KẾT THÚC PHẦN THÊM VÀO

    // Hằng số: Số lá bài mỗi người chơi
    final static int PLAYERCARD = 12;

    // Các biến static quản lý trạng thái game
    // Chỉ số người chơi hiện tại, tổng số người chơi, người chơi đang dẫn đầu
    static int currentPlayerIdx, numberOfPlayer, currentLeadingPlayer = 0;

    // Người chơi hiện tại và mảng tất cả người chơi
...
// Phương thức thiết lập người chơi tiếp theo
    public static void setNextPlayer(int nextPlayer){
        ...
        // Cập nhật các lá bài trong vùng chung
    ...
    public static void initializeGame(int p){
        ...
        // Khởi tạo vùng chung để chứa các lá bài
...
//Phương thức start - khởi tạo giao diện và logic Game
    public void start(Stage primaryStage) {

        //PHẦN THÊM VÀO
        // Khởi tạo lại mainPane (nếu chưa có)
        mainPane = new BorderPane();

        // Lấy một quân bài từ bộ bài
        Card card = deck.drawCard();  // Lấy quân bài từ bộ bài (giả sử có phương thức drawCard)

        // Hiển thị quân bài và thông điệp tùy theo chế độ
        displayCard(card);  // Gọi phương thức displayCard để hiển thị quân bài
        displayMessage("Game Started!");  // Gọi phương thức displayMessage để hiển thị thông điệp

        // Tạo Scene với mainPane và kích thước màn hình
    ...
    private void displayCard(Card card) {
        if (isBasicMode) {
            // Chế độ Basic: Hiển thị quân bài dưới dạng text
            String cardText = card.toString();  // Lấy chuỗi mô tả quân bài từ phương thức toString() của Card
            Label cardLabel = new Label(cardText);  // Tạo Label với nội dung là quân bài
            mainPane.setCenter(cardLabel);  // Đặt Label vào vị trí trung tâm của mainPane
        } else {
            // Chế độ Graphics: Hiển thị quân bài dưới dạng hình ảnh
            Image cardImage = new Image(getCardImagePath(card));  // Lấy đường dẫn hình ảnh quân bài
            ImageView imageView = new ImageView(cardImage);
            mainPane.setCenter(imageView);  // Đặt hình ảnh quân bài vào trung tâm của mainPane
        }
    }

    private String getCardImagePath(Card card) {
        // Trả về đường dẫn hình ảnh của quân bài
        return "/path/to/images/" + card.toString() + ".png";  // Cần thay đổi đường dẫn hình ảnh tùy theo hệ thống
    }

    private void displayMessage(String message) {
        // Hiển thị thông điệp dưới dạng JavaFX Label
        Label messageLabel = new Label(message);
        mainPane.setBottom(messageLabel);  // Đặt thông điệp vào dưới màn hình (Bottom của BorderPane)
    }
        //KẾT THÚC PHẦN THÊM VÀO
    
        //Thiết lập giao diện người chơi
        ...
        primaryStage.setScene(scene);
        primaryStage.show();
        //Kết thúc thiết lập giao diện người chơi
        ...
        class SkipTurnHandler implements EventHandler<ActionEvent>{
			public void handle(ActionEvent e) {
                ...
                //Cấp nhật giao diện cho người chơi mới
                setNextPlayer(currentPlayerIdx);
                playerCaptureButton[currentPlayerIdx].setDisable(false);
			}
        ...
        //Lớp xử lý sự kiện chọn bài (capture)
        class CaptureHandler implements EventHandler<ActionEvent> {
            public void handle(ActionEvent e) {
                ...
                if(!playedCards.isEmpty() && GameRule.isStronger(playedCards, poolCards)){
                    ...
                    //Nếu thắng, tìm người chơi tiếp theo còn active
                    playerCaptureButton[currentPlayerIdx].setDisable(true);
                    skipTurnButton[currentPlayerIdx].setDisable(true);
                    do {
                    	currentPlayerIdx = (currentPlayerIdx + 1)%numberOfPlayer;
    ...
    public static void main(String[] args){
        ...
        /*CHỌN CHẾ ĐỘ
        // Lựa chọn chế độ hiển thị: true cho chế độ Basic, false cho chế độ Graphics
        Game game = new Game(true);  // Chế độ Basic
        Game game = new Game(false);  // Chế độ Graphics
        */
        launch(args);
    }
}